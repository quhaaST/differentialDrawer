...
class RungeKuttaMethodSolution : NumericalSolutionBase() {
    override fun calculateSolutions() {
        ...
        for (i in 1 until initialStepNumber) {
            ...
            val k2 = initialEquation(currentX + stepSize / 2,
                    lastPair.second + stepSize * k1 / 2)
            val k3 = initialEquation(currentX + stepSize / 2,
                    lastPair.second + stepSize * k2 / 2)
            ...
            if (currentX - stepSize <= problemPoint && currentX > problemPoint) {
                newValue = ExactSolution().updateParameters(initialX, initialY, finalX,
                        initialStepNumber).getSolution(currentX)
            }
